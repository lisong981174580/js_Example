...
/*30、向上滚动执行上shang()   向下滚动执行xia()
obj是通过选择器选择到的元素  shang和xia是两个函数名*/
...
//33、日期函数
//"距离"+year+"年"+month+"月"+day+"日"+h+"时"+m+"分"+s+"秒还有："+day1+"天"+hour+"小时"+minute+"分"+second+"秒"; 
//参数为:年、月、日、时、分、秒、对象                对象用的是id选择器  所以应该有id名

   function daojishi(year,month,day,h,m,s,divname) {
			
			
			window.setInterval(function(){
				
				var now = new Date(); //当前事件
			    var endDate = new Date(year, month-1, day,h,m,s);  //要计算的终止事件
				var leftTime=endDate.getTime()-now.getTime();   //时间差值
			    var leftsecond = parseInt(leftTime/1000);   //算出秒数
				var day1=Math.floor(leftsecond/(60*60*24));   //根据秒数 算出天数
				var hour=Math.floor((leftsecond-day1*24*60*60)/3600);   //根据剩余的秒数  算出小时数
				var minute=Math.floor((leftsecond-day1*24*60*60-hour*3600)/60);   //根据剩余的秒数 算出 分数
				var second=Math.floor(leftsecond-day1*24*60*60-hour*3600-minute*60);   //根据剩余的秒数 算出   秒数
				var cc = document.getElementById(divname); 
				cc.innerHTML = "距离"+year+"年"+month+"月"+day+"日"+h+"时"+m+"分"+s+"秒还有："+day1+"天"+hour+"小时"+minute+"分"+second+"秒"; 
...
//画成一个带有刻度的表盘，里面可以不断的填表盘，使用时应该有放他的父元素
//半径   对象   针宽度  针高度   颜色           对象是字符串型  用的是类型选择器      body=document.querySelectorAll(obj)[0];
	function  Time(r,obj,zhen_width,zhen_height,color){
			...
			for(i=0;i<60;i++){
				...
				hu=(Math.PI/180)*deg;
				 ...
				 var  body=document.querySelectorAll(obj)[0];
				 body.style.cssText="display: flex;justify-content: center;align-items: center;"
				 body.style.width=2*r+"px";
				 body.style.height=2*r+"px";
				 body.style.position="relative";
				 
				 var  shizhen=document.createElement("div");
				 body.appendChild(shizhen);
				 
				var x=r*Math.cos(hu)+r;
				var y=r*Math.sin(hu)+r;
				shizhen.style.cssText="width:"+zhen_width+"px;height:"+zhen_height+"px;background: "+color+";  position: absolute; transform:rotate("+(deg+90)+"deg);  top:"+y+"px;  left:"+x+"px ;";
